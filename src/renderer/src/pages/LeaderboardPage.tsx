    ...
    <div className="h-full w-full overflow-y-auto" style={{ background: 'radial-gradient(130% 100% at 50% -10%, #14223a 0%, #0a1120 45%, #060912 100%)' }}>
      ...
      <div className="max-w-5xl mx-auto px-6 py-6">
        ...
        <div className="flex items-center gap-3 mb-1">
          ...
          <h1 className="text-xl font-black text-[#c9a227] uppercase tracking-[0.2em]">Classement</h1>
          <span className="ml-auto flex items-center gap-2 text-[12px] text-white/55">
            ...
            <b className="text-emerald-400 font-mono">{fmt(online)}</b> en ligne
          </span>
        </div>
        <p className="text-[11px] text-white/35 mb-4">{fmt(roster.length + 1)} joueurs classés · saison en cours</p>
          ...
          <div className="flex items-center gap-6 px-4 py-2 whitespace-nowrap animate-[scroll_30s_linear_infinite] hover:[animation-play-state:paused]">
            {[...wins, ...wins].map((w, i) => (
              <span key={i} className="inline-flex items-center gap-1.5 text-[11px] text-white/55">
                <Flame size={12} className="text-orange-400" /> {w.flag} <b className="text-white/80">{w.name}</b> remporte <b className="text-emerald-400">+${fmt(w.amount)}</b> <span className="text-white/30">({w.kind})</span>
              </span>
            ))}
          </div>
        </div>
          ...
          {[1, 0, 2].map((idx, col) => {
            ...
            return (
              <motion.div key={p.id} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: col * 0.06 }}
                ...
                <p className="text-[9px] text-white/35">ROI +{p.roi}% · {fmt(p.hands)} mains</p>
              </motion.div>
            )
          })}
        </div>
        ...
        <div className="mb-4 rounded-2xl border border-[#c9a227]/40 bg-[#c9a227]/[0.07] p-4 flex items-center gap-4">
          ...
          <div className="flex-1">
            <p className="text-[11px] uppercase tracking-widest text-white/40 font-bold">Ton rang</p>
          ...
          <div className="text-right">
            ...
            <p className="text-[10px] text-white/35">sur {fmt(hero.total)} · rating {fmt(heroRating)}</p>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-white/10 overflow-hidden">
          <div className="grid grid-cols-[48px_1fr_70px_70px_80px] gap-2 px-4 py-2 bg-white/[0.04] text-[9px] uppercase tracking-widest text-white/40 font-bold">
            <span>#</span><span>Joueur</span><span className="text-right">Rating</span><span className="text-right">ITM</span><span className="text-right">ROI</span>
          </div>
          <div className="divide-y divide-white/5">
            {list.map((p: RosterPlayer) => (
              <div key={p.id} className="grid grid-cols-[48px_1fr_70px_70px_80px] gap-2 px-4 py-2 items-center hover:bg-white/[0.03] transition-colors">
                ...
                <span className="flex items-center gap-2 min-w-0">
                  {isOnline(p.id, bucket) && <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 flex-shrink-0" title="En ligne" />}
            ...
            {list.length === 0 && <div className="px-4 py-6 text-center text-[12px] text-white/30">Aucun joueur trouvé.</div>}
          </div>
        </div>

        <div className="flex items-center justify-center gap-4 mt-4 text-[10px] text-white/30">
          <span className="flex items-center gap-1"><Users size={11} /> {fmt(roster.length + 1)} joueurs</span>
          <span className="flex items-center gap-1"><TrendingUp size={11} /> Top 100 affiché</span>
        </div>
        <div className="h-4" />
      </div>
    </div>
