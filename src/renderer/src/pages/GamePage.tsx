  ...
  for (let i = 0; i <= stepIdx && i < record.actions.length; i++) {
    ...
    if (a.actionType !== 'SB' && a.actionType !== 'BB') {
      ...
      ranges[a.seatIdx] = applyAction(ranges[a.seatIdx] ?? initRange(board), cat, {
        ...
        // size of a BET or RAISE as a fraction of the pot → a big size polarizes the
        // read, a small one merges it.
        betFrac: !preflop && cat === 'aggr' && pot > 0 ? a.amount / pot : undefined,
      })
    }
...
export default function GamePage(): JSX.Element {
  ...
  function getSeatPosPct(idx: number, total: number): { left: string; top: string; transform: string } {
    ...
    // Lift the hero (offset 0, bottom-centre) just a touch so the stack clears the
    // floating action bar — but keep it close to aligned with the other seats (the bar
    // sits at the very bottom edge, so a small lift is enough).
    if (offset === 0) y -= compactTable ? 11 : 6
  ...
  function trackRange(seatIdx: number, action: string, currentGs: GState, amount?: number) {
    ...
    const ctx = {
      ...
      // size of a BET or RAISE as a fraction of the pot before it → a big size polarizes
      // the read (medium hands cut), a small one merges it (thin value included).
      betFrac: !preflop && cat === 'aggr' && (currentGs.pot - seat.bet) > 0 ? seat.bet / (currentGs.pot - seat.bet) : undefined,
    }
    ...
    <div className="relative flex flex-col h-full overflow-hidden select-none" style={{background:'#0c0907'}}>
      ...
      <div className="flex-1 relative overflow-hidden min-h-0"
        ...
        {/* ── Soft animated tournament-hall backdrop (tournament only) ──
            Layered just above the Room base but BELOW the felt/seats/cards
            (which come later in the DOM), so the hall is only ever visible
            AROUND the table, never under it. Slow Ken-Burns breathing. */}
        {tournament && (
          <div className="absolute inset-0 pointer-events-none overflow-hidden">
            ...
            {/* Centre vignette — focuses the eye on the table */}
