...
export default function DecisionTrainer({ onBack }: { onBack: () => void }) {
  ...
  if (phase === 'setup') {
    return (
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md mt-8 rounded-2xl border border-white/10 bg-white/[0.02] p-6">
        ...
        <h2 className="font-black uppercase tracking-widest text-sm mb-1" style={{ color: ACCENT }}>Le bon coup</h2>
        <p className="text-[11px] text-white/40 mb-5 leading-relaxed">Face à l'action adverse (sa <b className="text-white/60">taille de mise</b>), choisis le <b className="text-white/60">bon coup</b> : fold / call / raise / bet / check. Le coach révèle ensuite sa range, tes 2 questions en or, et le tell de sizing.</p>

        <label className="text-[10px] uppercase tracking-widest text-white/40 font-bold">Contexte</label>
        <div className="grid grid-cols-2 gap-2 mt-2 mb-4">
          {([['cash', 'Cash 100bb'], ['mtt', 'Tournoi MTT']] as const).map(([id, lbl]) => (
        ...
        <div className="grid grid-cols-2 gap-2 mt-2 mb-4">
          {([['chrono', true, 'Réflexe (chrono 8s)'], ['calme', false, 'Sans chrono']] as const).map(([id, val, lbl]) => (
  ...
  if (phase === 'result') {
    ...
    return (
      <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-md mt-10 rounded-2xl border border-white/10 bg-white/[0.02] p-8 text-center">
        ...
        <h2 className="text-white/80 font-black uppercase tracking-widest text-sm">Résultat — Le bon coup</h2>
        ...
        <p className="text-sm font-bold mb-5" style={{ color: ratio >= 0.8 ? '#4ade80' : ratio >= 0.6 ? '#fbbf24' : '#f87171' }}>
          {Math.round(ratio * 100)}% — {ratio >= 0.8 ? 'Réflexes affûtés 🔥' : ratio >= 0.6 ? 'Ça vient' : 'À retravailler'}
        </p>
        ...
        <div className="flex gap-3">
          <button onClick={start} className="flex-1 py-2.5 rounded-xl text-[12px] font-black uppercase tracking-widest" style={{ background: `linear-gradient(135deg,${ACCENT},#c9781a)`, color: '#2a1604' }}>Rejouer</button>
  ...
  const prompt = spot.facingBet
    ? `L'adversaire MISE ${bbU(spot.toCall)}bb (~${Math.round(spot.betFrac * 100)}% du pot). À toi de jouer.`
    : `L'adversaire CHECKE. C'est à toi (personne n'a misé).`
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="w-full max-w-4xl mt-4">
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-4 mt-3">
        <p className="text-[10px] uppercase tracking-widest font-bold mb-1" style={{ color: ACCENT }}>Le bon coup ?</p>
          ...
          <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}>
            <div ref={explainRef} className="mt-3 p-3.5 rounded-xl border"
              ...
              <p className="text-[11px] uppercase tracking-widest font-black mb-1.5" style={{ color: picked === spot.correct ? '#4ade80' : '#fbbf24' }}>
                {picked === spot.correct ? '✅ Bien joué' : picked === ('—' as AdviceAction) ? '⏱ Temps écoulé' : '💡 Le bon coup'} : <span style={{ color: ACCENT }}>{rev.correctLabel}</span> ({spot.advice.sizingText})
              </p>
              <p className="text-[12.5px] text-white/85 leading-relaxed mb-1.5">{rev.lesson}</p>
              <p className="text-[11.5px] leading-relaxed mb-1.5" style={{ color: '#cbe9d8' }}><b style={{ color: '#34d399' }}>Tes 2 questions :</b> {rev.twoQuestions}</p>
              ...
              <p className="text-[10px] text-white/45 mt-1.5">Équité ≈ {Math.round(rev.equity * 100)}% · seau : <b className="text-white/65">{rev.bucket}</b></p>
            </div>
            <button onClick={next} className="w-full mt-3 flex items-center justify-center gap-2 py-2.5 rounded-xl font-black uppercase tracking-[0.18em] text-[12px]"
              style={{ background: `linear-gradient(135deg,${ACCENT},#c9781a)`, color: '#2a1604' }}>
              {handNo + 1 < numHands ? <>Main suivante</> : <><Check size={14} /> Voir le résultat</>}
            </button>
          </motion.div>
        )}
      </div>
